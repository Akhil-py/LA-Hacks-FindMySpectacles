...
//   @input Asset.MeshAsset coneMesh
...
async function createDetectionPrefabAsync(detection) {
  ...
  if (script.coneMesh) {
      // 2) spawn the arrow
      const arrow = scene.createSceneObject("Arrow");
      const meshVis = arrow.createComponent("Component.MeshVisual");
      meshVis.mesh = script.coneMesh;
    
      // 3) position it at the same spot (or slightly offset if you like)
      const arrowT = arrow.getTransform();
      arrowT.setWorldPosition(newCenter);
    
      // 4) point its forward axis at the detection
      //    by convention, lookAt points the +Z axis of the object toward the target
      arrowT.lookAt(
        detectionObj.getTransform().getWorldPosition(),   // target position
