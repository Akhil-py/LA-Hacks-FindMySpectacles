...
let AnchorPlacementController = class AnchorPlacementController extends BaseScriptComponent {
    ...
    async setupAnchor() {
        ...
        let anchorPosition = toWorldFromDevice.mult(mat4.fromTranslation(new vec3(0, 0, 0)));
        // Create the anchor
        this.anchor = await this.anchorSession.createWorldAnchor(anchorPosition);
        // Save the anchor so it's loaded in future sessions
        try {
            this.anchorSession.saveAnchor(this.anchor);
        }
        catch (error) {
