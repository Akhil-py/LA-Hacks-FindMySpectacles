...
//@input Component.ScriptComponent anchorController
...
//      // 1) make a copy of your cone‐prefab
//      const arrow = script.arrowPrefab.instantiate(scene.createSceneObject("Arrow"));
//    
//      // 2) put it 1 m in front of the camera
//      const cam = script.pinholeCapture.getSceneObject().getTransform();
//      const spawnPos = cam.getWorldPosition().add(cam.forward.uniformScale(1.0));
//      arrow.getTransform().setWorldPosition(spawnPos);
//    
//      // 3) scale it so you can actually see it
//      arrow.getTransform().setLocalScale(new vec3(0.2, 0.2, 0.2));
//    
//      // 4) now point its +Z axis at the detection’s world position
//      const targetPos = detectionObj.getTransform().getWorldPosition();
//      arrow.getTransform().lookAt(targetPos, new vec3(0,1,0));
//    
//      debugLog("Spawned arrow at " + spawnPos.toString() + " pointing at " + targetPos.toString());
//      activePrefabs.push(arrow);
//}
        
    const worldMat = detectionObj.getTransform().getWorldTransform();
    
    script.anchorController.createAnchor(worldMat);
    debugLog(worldMat.toString());
//    var temp = event.interactor.targetHitInfo.hit.position;
//    anchor.createAnchor(worldMat);
        
    debugLog(`World transform matrix (mat4): ${JSON.stringify(worldMat)}`);
...
async function processDetections(detections) {
  ...
  const validDetections = detections.filter(
    ...
    && detection.class_name.toLowerCase() === "bottle"
  );
  ...
  for (let i = 0; i < validDetections.length; i++) {
        // IM EDITING THIS TO ONLY DO ONE
    debugLog(
