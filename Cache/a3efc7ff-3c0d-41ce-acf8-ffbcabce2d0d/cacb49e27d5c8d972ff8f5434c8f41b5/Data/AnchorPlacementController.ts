...
//    this.attachNewObjectToAnchor(anchor);
  ...
  private moveTD(anchor: Anchor) {
    var anchorInfo = LocationAsset.getGeoAnchoredPosition(
        script.longitude,
        script.latitude
        );
    var worldPos = anchorInfo.position;
    this.myObject.getTransform().setWorldPosition(worldPos);
  }
}